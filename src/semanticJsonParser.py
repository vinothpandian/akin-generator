...
class SemanticJsonParser:
    ...
    def remove_sub_elements(element_bounds):
        within = []
        nothing_within = ["text","log_in","sign_up","username","password","icon","forgot","sm_button","button",
                          "privacy","check","name","navigation_dots","number","selector","search","edit_number",
                          "edit_string","filter","top_bar","heart_icon","sort","rating","bottom_bar","cart_add","buy"]
        remove = ["other"]
    ...
    def mark_image(img, meta, label_hierarchy_map, label_color_map, border=True, gan_output=False, skip_box=False):
        ...
        for box in meta:
            thickness = -1
            color, element = SemanticJsonParser.get_element_color(box[0], label_color_map, label_hierarchy_map, gan_output)
            #print(color, element)
    ...
    @staticmethod
    def convert_bb_to_image(predictions_bb, predictions_label, img_h, img_w, label_hierarchy_map, label_color_map, dst,
                            ctr=0, combined=False):
        ...
        for i in range(len(predictions_bb)):
            ...
            if combined:
                ...
                img = SemanticJsonParser.remove_top_bottom(img)
                SemanticJsonParser.save_img(img, path=os.path.join(dst, "predicted_"+str(ctr)+"_"+str(i)+".jpg"))
        if combined:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--source_folder", "-s", help="path to annotation json files",
                        default="../../../data/folder_labels")
    parser.add_argument("--destination_path", "-d", help="path to destination folder",
                        default="../../../data/semantic_all")
